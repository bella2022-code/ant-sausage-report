(() => {
  ...
  const translations = {
    'zh-Hant': {
      ...
      controls:'手機：點一下目的地　·　電腦：拖曳、WASD、方向鍵、空白鍵跳躍、E 咬斷', next:'下一關 →', jump:'跳！', bridge:'架橋！', livingBridge:'活體橋！', bridgeLeaves:'先找齊三片葉子，才能架橋。', bridgeNear:'靠近大水窪後按「架橋！」', bridgeBuilt:'螞蟻們把葉子排成了一座活橋！', livingBridgeHint:'靠近水道後，讓幾隻螞蟻搭成活體橋。', livingBridgeBuilt:'6 隻螞蟻留在水面，架起活體橋！', bite:'咬斷', split:'分碎', bigWater:'超大水窪', jumpHint:'靠近水邊時按「跳！」或空白鍵', cutHint:'竹籤還卡著！按「咬斷」或 E。', splitHint:'食物太大了！按「分碎」或 E。', doneTag:'已完成', aria:'017 火腿腸報告遊戲。點擊或拖曳主角螞蟻移動。',
    ...
    en: {
      ...
      controls:'Mobile: tap a destination · Desktop: drag, WASD, arrows, Space to jump, E to bite', next:'Next level →', jump:'JUMP!', bridge:'BRIDGE!', livingBridge:'LIVING BRIDGE!', bridgeLeaves:'Find all three leaves before building the bridge.', bridgeNear:'At the giant puddle, press BRIDGE!', bridgeBuilt:'The colony arranges the leaves into a living bridge!', livingBridgeHint:'At the waterway, let a few ants form a living bridge.', livingBridgeBuilt:'Six ants stay behind to form a living bridge!', bite:'BITE', split:'SPLIT', bigWater:'GIANT PUDDLE', jumpHint:'At the edge, press JUMP or Space', cutHint:'The bamboo skewer is stuck. Press BITE or E.', splitHint:'This food is too large. Press SPLIT or E.', doneTag:'Complete', aria:'017 Sausage Report game. Tap or drag to move the scout ant.',
    ...
    ja: {
      ...
      controls:'スマホ：目的地をタップ　·　PC：ドラッグ、WASD、矢印キー、Spaceでジャンプ、Eでかみ切る', next:'次のステージ →', jump:'ジャンプ！', bridge:'橋を作る！', livingBridge:'生きた橋！', bridgeLeaves:'葉っぱを3枚集めてから橋を作ろう。', bridgeNear:'大きな水たまりで「橋を作る！」を押そう。', bridgeBuilt:'仲間が葉っぱを並べて、生きた橋になった！', livingBridgeHint:'水路のそばで、仲間に生きた橋を作ってもらおう。', livingBridgeBuilt:'6匹が水面に残り、生きた橋になった！', bite:'かみ切る', split:'分ける', bigWater:'大きな水たまり', jumpHint:'水ぎわでジャンプかSpaceを押す', cutHint:'竹串が刺さっている！かみ切るか E を押す。', splitHint:'食べ物が大きすぎる！分けるか E を押す。', doneTag:'クリア', aria:'017 ソーセージ報告ゲーム。タップまたはドラッグで偵察アリを動かします。',
  ...
  const levelMenuCopy = {
    'zh-Hant': [
      ...
      ['月光香腸','火腿腸 · 夜巡與竹籤'], ['彎月急轉','彎彎火腿 · 直接救援'],
      ['肉丸真假','肉丸 · 假目標與分碎'], ['紅螞蟻十字路','愛心火腿 · 中央地盤'],
    ...
    en: [
      ...
      ['Moonlit Sausage','Sausage · night patrol & skewer'], ['Crescent Sprint','Curly ham · direct rescue'],
      ['Meatball Misdirect','Meatball · decoy & split'], ['Red Ant Crossroads','Heart ham · central territory'],
    ...
    ja: [
      ...
      ['月明かりのソーセージ','ソーセージ · 夜の巡回と竹串'], ['三日月スプリント','曲がったハム · すぐに救援'],
      ['肉だんごの迷わせ','肉だんご · おとりと分ける'], ['赤アリの交差点','ハートハム · 中央の縄張り'],
  ...
  const levels = [
    ...
    { nest:{ x:480, y:458, r:48 }, food:{ x:130, y:115, shape:'meatball', key:'foodMeatball', radius:42 }, rock:{ x:275, y:285, r:64 }, puddle:{ x:674, y:360, rx:112, ry:54 }, rival:{ x:730, y:136, r:72 } },
    ...
    { nest:{ x:114, y:270, r:64 }, food:{ x:842, y:270, shape:'donut', key:'foodDonut', radius:55 }, rock:{ x:746, y:112, r:54 }, puddle:{ x:480, y:270, rx:165, ry:370, huge:true, bridge:true, livingBridge:true }, rival:{ x:746, y:432, r:74 } },
    ...
    { mode:'night', patrol:true, nest:{ x:118, y:426, r:54 }, food:{ x:838, y:118, shape:'sausage', key:'foodSausage', radius:52, skewer:true }, rock:{ x:484, y:286, r:71 }, puddle:{ x:292, y:184, rx:114, ry:52 }, rival:{ x:706, y:298, r:80 } },
    { mode:'rush', nest:{ x:842, y:122, r:60 }, food:{ x:116, y:424, shape:'crescent', key:'foodCrescent', radius:58 }, rock:{ x:478, y:292, r:72 }, puddle:{ x:628, y:386, rx:178, ry:80, huge:true }, rival:{ x:716, y:176, r:78 } },
    { mode:'decoy', decoy:{x:512,y:112}, nest:{ x:110, y:426, r:48 }, food:{ x:842, y:404, shape:'meatball', key:'foodMeatball', radius:42, fragment:true }, rock:{ x:414, y:372, r:68 }, puddle:{ x:310, y:192, rx:108, ry:50 }, rival:{ x:642, y:246, r:82 } },
    { nest:{ x:850, y:270, r:68 }, food:{ x:114, y:270, shape:'heart', key:'foodHeart', radius:62 }, rock:{ x:484, y:140, r:70 }, puddle:{ x:480, y:410, rx:134, ry:60 }, rival:{ x:484, y:304, r:78 } },
    { nest:{ x:112, y:270, r:62 }, food:{ x:846, y:270, shape:'cheese', key:'foodCheese', radius:48 }, rock:{ x:740, y:112, r:52 }, puddle:{ x:480, y:270, rx:160, ry:370, huge:true, bridge:true }, bridgeLeaves:[{x:210,y:126,a:-.5},{x:338,y:416,a:.7},{x:744,y:168,a:.2}], rival:{ x:740, y:432, r:76 } },
    { mode:'night', patrol:true, nest:{ x:846, y:424, r:45 }, food:{ x:118, y:118, shape:'square', key:'foodSquare', radius:46, skewer:true, fragment:true }, rock:{ x:478, y:264, r:70 }, puddle:{ x:690, y:180, rx:112, ry:50 }, rival:{ x:330, y:392, r:80 } },
    { mode:'rush', nest:{ x:120, y:118, r:66 }, food:{ x:836, y:422, shape:'heart', key:'foodHeart', radius:62 }, rock:{ x:510, y:300, r:72 }, puddle:{ x:272, y:386, rx:114, ry:52 }, rival:{ x:664, y:158, r:80 } },
  ...
  function setUi() {
    ...
    bridgeButton.textContent = puddle?.livingBridge ? t('livingBridge') : t('bridge');
  ...
  function insideBoard(point, padding = 0) { return point.x >= padding && point.x <= W - padding && point.y >= padding && point.y <= H - padding; }
  ...
  function update(dt, now) {
    ...
    if (state.phase === 'carry' && carryTarget) {
      ...
      if (d > 9) {
        const teamSpeed = state.bridgeAnts ? .68 : 1;
        const step = Math.min(d, (inSticky(s) ? 58 : 120) * teamSpeed * dt / 1000);
        ...
        if (insideBoard(next, currentFoodRadius()) && distance(next, rock) > rock.r + currentFoodRadius() && (!ellipseContains(next.x, next.y) || state.bridgeBuilt) && !rivalTooNear && !propTooNear) {
          s.x = next.x; s.y = next.y; s.a = Math.atan2(dy, dx);
        } else if (!state.notice || now > state.notice.until - 500) {
          state.notice = { text: distance(next, rock) <= rock.r + currentFoodRadius() ? t('rock') : rivalTooNear ? t('rival') : propTooNear ? t('sceneBlock') : puddle.livingBridge ? t('livingBridgeHint') : puddle.bridge ? t('bridgeNear') : t('puddle'), until: now + 1050 };
        }
      }
  ...
  function drawBridge() {
    ...
    if (puddle.livingBridge) {
      const count = state.bridgeAnts || 6;
      for (let i = 0; i < count; i++) {
        const progress = i / (count - 1);
        drawAnt(puddle.x - puddle.rx + 25 + progress * (puddle.rx * 2 - 50), puddle.y + Math.sin(i * 1.8) * 7, 0, .62);
      }
      label(locale === 'en' ? 'LIVING BRIDGE' : locale === 'ja' ? '生きた橋' : '活體橋', puddle.x, puddle.y - 25, 13, '#fffaf2');
      return;
    }
  ...
  function drawTeam(now) {
    ...
    grips.forEach(([gx,gy,dir],i)=>{
      if (state.bridgeAnts && i < state.bridgeAnts) return;
  ...
  function tryBuildBridge() {
    ...
    if (!puddle.livingBridge && bridgeLeaves.some(leaf => !leaf.collected)) { state.notice = { text:t('bridgeLeaves'), until:performance.now() + 1200 }; return; }
    const s = state.sausage;
    const nearPuddle = ((s.x - puddle.x) ** 2) / ((puddle.rx + 105) ** 2) + ((s.y - puddle.y) ** 2) / ((puddle.ry + 105) ** 2) < 1;
    if (!nearPuddle) { state.notice = { text:puddle.livingBridge ? t('livingBridgeHint') : t('bridgeNear'), until:performance.now() + 1200 }; return; }
    state.bridgeBuilt = true; state.bridgeAnts = puddle.livingBridge ? 6 : 0; bridgeButton.hidden = true;
    state.notice = { text:puddle.livingBridge ? t('livingBridgeBuilt') : t('bridgeBuilt'), until:performance.now() + 1800 };
  }
